...
{
    class LedManager_NVD_Back : LedManager
    {
        private Thread t;
        ...
        protected override void UpdateLed_NoAnimation()
        {
            ...
            {
                _NDA.NDA_SetIlluminationParmColor_RGB(adapterIndexes[i], 21, 2, 0, 0, 0, 4, 0, 0, ledColor.R, ledColor.G, ledColor.B, true);
            }
            this.PatientlyWait();
        }
        ...
        protected override void UpdateLed_Breathing()
        {
            for (int i = 0; i < this.adapterIndexes.Count; i++)
            {
                _NDA.NDA_SetIlluminationParmColor_RGB(adapterIndexes[i], 27, 2, 0, 0, 0, 4, 0, 0, ledColor.R, ledColor.G, ledColor.B, false);
            }
            this.PatientlyWait();
        }
        ...
        protected override void UpdateLed_Flashing()
        {
            for (int i = 0; i < this.adapterIndexes.Count; i++)
            {
                _NDA.NDA_SetIlluminationParmColor_RGB(adapterIndexes[i], 28, 2, 0, 100, 100, 4, 0, 0, ledColor.R, ledColor.G, ledColor.B, false);
            }
            this.PatientlyWait();
        }
        ...
        protected override void UpdateLed_DoubleFlashing()
        {
            for (int i = 0; i < this.adapterIndexes.Count; i++)
            {
                _NDA.NDA_SetIlluminationParmColor_RGB(adapterIndexes[i], 30, 2, 0, 10, 15, 4, 2, 0, ledColor.R, ledColor.G, ledColor.B, false);
            }
            this.PatientlyWait();
        }
        ...
        protected override void UpdateLed_Off()
        {
            for (int i = 0; i < this.adapterIndexes.Count; i++)
            {
                _NDA.NDA_SetIlluminationParmColor_RGB(adapterIndexes[i], 24, 2, 0, 0, 0, 0, 0, 0, ledColor.R, ledColor.G, ledColor.B, true);
            }
            this.PatientlyWait();
        }
